        ...
        const fetchClassRoom = async () => {
          try {
            const res1 = await axios.get(
              `${process.env.REACT_APP_SERVER_URL}/api/classroom?userId=${userId}&classId=${classroomList[i].classId}`
            );
          ...
          setLinkData([
            {
              ...linkData,
              [playlist_id]:
                classTempData[i].lectures[j].contents[k].playlistId,
              [contentName]:
                classTempData[i].lectures[j].contents[k].contentName,
            },
          ]);
