...
const AcceptedPart = (props) => {
    ...
    const fetchAcceptedList = async (cid) => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/api/classroom/accepted-list?classId=${cid}`);
            // console.log(response.data);
            for (const prop in response.data) {
                                          ...
                                          acceptedList.map((accepted) => (
                                              <tr>
                                                  {managedClassroom.map((classroom) => {
                                                      ...
                                                      if (accepted.classId === classroom.classId) return <td>{classroom.className}</td>;
                                                  })}
