...
const RejectedPart = (props) => {
    ...
    const fetchRejectedList = async (cid) => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/api/classroom/rejected-list?classId=${cid}`);
            // console.log(response.data);
            for (const prop in response.data) {
                                          ...
                                          rejectedList.map((rejected) => (
                                              <tr>
                                                  {managedClassroom.map((classroom) => {
                                                      ...
                                                      if (rejected.classId === classroom.classId) return <td>{classroom.className}</td>;
                                                  })}
